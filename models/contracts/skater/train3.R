...
TERM_OUT <- 'models/contracts/skater/term3.rds'
AAVP_OUT <- 'models/contracts/skater/aavP3.rds'
N_FOLDS <- 5L
TERM_GRID_SIZE <- 15L
AAVP_GRID_SIZE <- 25L
...
# Plot top feature importances from fitted LightGBM model.
...
# ----- Train Term Model (LightGBM Classification) ----- #
...
term_spec <- parsnip::boost_tree(
  ...
  parsnip::set_engine('lightgbm', num_threads = 1, verbose = -1)
...
# ----- Train AAV% Model (LightGBM Regression) ----- #
...
plot_lgbm_importance(
  ...
  model_name = 'Term (LightGBM)'
)
plot_lgbm_importance(
  fitted_wf = aavp_fit_obj$fit,
  model_name = 'AAV% (LightGBM)'
)
