...
TERM_OUT <- 'models/contracts/skater/term2.rds'
AAVP_OUT <- 'models/contracts/skater/aavP2.rds'
N_FOLDS <- 5L
TERM_GRID_SIZE <- 15L
AAVP_GRID_SIZE <- 25L
...
term_spec <- parsnip::boost_tree(
  ...
  parsnip::set_engine('xgboost', eval_metric = 'mlogloss', nthread = 1)
...
aavp_spec <- parsnip::boost_tree(
  ...
  parsnip::set_engine('xgboost', eval_metric = 'rmse', nthread = 1)
...
plot_xgb_importance(
  ...
  model_name = 'Term (XGBoost)'
)
plot_xgb_importance(
  fitted_wf = aavp_fit_obj$fit,
  model_name = 'AAV% (XGBoost)'
)
