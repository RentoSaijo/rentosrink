...
select_preferred_predictions <- function(predictions, preferred_engines) {
  preferred_tbl <- tibble::tibble(
    dataset = names(preferred_engines),
    engine = unname(preferred_engines)
  ) %>%
    dplyr::mutate(
...
selected_predictions <- select_preferred_predictions(predictions, PREFERRED_ENGINES)
