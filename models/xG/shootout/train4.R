...
shots <- pbps %>%
  ...
  dplyr::filter(
    ...
    # Keep only shootouts and penalty shots.
    situationCode %in% c('1010', '0101')
  ) %>%
  dplyr::mutate(
    ...
    shooterSide          = factor(shooterSide),
    shooterPositionCode  = factor(shooterPositionCode),
    goalieSide           = tidyr::replace_na(goalieSide, 'neutral'),
    goalieSide           = factor(goalieSide),
...
saveRDS(out, file = 'models/xG/shootout/model4.rds')
