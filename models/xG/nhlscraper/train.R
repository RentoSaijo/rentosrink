...
run_ridge_training <- function(dataset) {
  model_key <- paste0(dataset, "_ridge")
  ...
  repeat {
    ...
    if (!is.na(boundary_hit) &&
      ...
      cat(
        sprintf(
          "Accepting lower-bound penalty %.8f for %s at log10 lower bound %.1f.\n",
          best_params$penalty,
          dataset,
          current_penalty_range[[1]]
        )
      )
      break
    }
    ...
    cat(
      sprintf(
        "Best penalty %.8f hit the %s boundary for %s; expanding log10 range from [%.1f, %.1f].\n",
        best_params$penalty,
        boundary_hit,
        dataset,
        current_penalty_range[[1]],
        current_penalty_range[[2]]
      )
    )
  ...
  readr::write_csv(best_params_output, file.path(results_dir, paste0(model_key, "_best_params.csv")))
  ...
  readr::write_csv(coefficients, file.path(results_dir, paste0(model_key, "_coefficients.csv")))
  readr::write_csv(search_history, file.path(results_dir, paste0(model_key, "_search_history.csv")))
  readr::write_csv(
    preprocessing$unknown_levels,
    file.path(results_dir, paste0(model_key, "_unknown_levels.csv"))
  )
  readr::write_csv(
    preprocessing$novel_levels,
    file.path(results_dir, paste0(model_key, "_novel_levels.csv"))
  )
  readr::write_csv(
    preprocessing$dummy_levels,
    file.path(results_dir, paste0(model_key, "_dummy_levels.csv"))
  )
  readr::write_csv(
    preprocessing$impute_medians,
    file.path(results_dir, paste0(model_key, "_impute_medians.csv"))
  )
  readr::write_csv(
    preprocessing$normalize_params,
    file.path(results_dir, paste0(model_key, "_normalize_params.csv"))
  )
  readr::write_csv(
    preprocessing$zero_variance_terms,
    file.path(results_dir, paste0(model_key, "_zero_variance_terms.csv"))
  )
  ...
  cat(sprintf("Saved fitted workflow to models/xG/nhlscraper/%s.rds\n", model_key))
  cat(sprintf("Saved coefficients to models/xG/nhlscraper/results/%s_coefficients.csv\n", model_key))
}
