...
LOWER_IS_BETTER = {'GoalsA', 'xGoalsA'}
DISPLAY_METRICS = ['SavePct', 'GoalsA', 'xGoalsA', 'GSAx']
...
def _prepare_biographies(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    return bio.loc[bio['positionCode'] == 'G'].copy()
...
def _prepare_shots(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    shots = shots.dropna(subset=['goaliePlayerId', 'gameId', 'eventId']).copy()
    shots['goaliePlayerId'] = shots['goaliePlayerId'].astype('int64')
...
def _load_page_data(season_id: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    basic = _prepare_basic(load_gbgs_goalie_basic(season_id))
    advanced = _prepare_advanced(load_gbgs_goalie_advanced(season_id))
    shots = _prepare_shots(load_sbss_goalies(season_id))

    goalie_games = basic.merge(
        advanced,
        on=['playerId', 'gameId'],
        how='left',
        suffixes=('', '_adv'),
    ...
    goalie_games = goalie_games.merge(game_meta, on='gameId', how='left')
    shots = shots.merge(game_meta, on='gameId', how='left')

    return games, goalie_games, shots
...
def _goalie_metric_series(df_in: pd.DataFrame, metric_name: str, situation_code: str) -> pd.Series:
    shots_against = _sum_metric(df_in, 'sA', situation_code)
    goals_against = _sum_metric(df_in, 'gA', situation_code)

    if metric_name == 'SavePct':
        save_pct = shots_against.sub(goals_against).div(shots_against.where(shots_against > 0)).mul(100.0)
        return save_pct.replace([float('inf'), float('-inf')], 0.0).fillna(0.0)
    if metric_name == 'GoalsA':
        return goals_against
    if metric_name == 'xGoalsA':
        return _sum_metric(df_in, 'xGA', situation_code)
    if metric_name == 'GSAx':
        return _goalie_metric_series(df_in, 'xGoalsA', situation_code) - goals_against
    if metric_name == 'CorsiA':
        return _sum_metric(df_in, 'cA', situation_code)
    if metric_name == 'FenwickA':
        return _sum_metric(df_in, 'fA', situation_code)
    if metric_name == 'ShotsA':
...
def _eligible_goalies(df_in: pd.DataFrame, situation_code: str) -> pd.DataFrame:
    pop = df_in.copy()
    if pop.empty:
        return pop
    pop['shotsAgainst'] = _goalie_metric_series(pop, 'ShotsA', situation_code)
    return pop.loc[pop['shotsAgainst'] > 0].copy()


def _default_goalie_id(df_in: pd.DataFrame, situation_code: str) -> int | None:
    if df_in.empty:
        return None
    ranking = _goalie_metric_series(df_in, 'GSAx', situation_code)
    if ranking.empty:
        return None
...
def _clamp_date_range(current_value, min_date: dt.date, max_date: dt.date) -> tuple[dt.date, dt.date]:
...
def _metric_label(metric_name: str) -> str:
    return GOALIE_METRIC_LABELS.get(metric_name, metric_name)


def _goalie_metric_value(metric_name: str, value: float) -> str:
...
def _goalie_delta(values: pd.Series, player_value: float, metric_name: str) -> float:
...
def _arc_points(cx: float, cy: float, radius: float, start_deg: float, end_deg: float, steps: int = 64) -> tuple[list[float], list[float]]:
    angles = [math.radians(start_deg + (end_deg - start_deg) * i / (steps - 1)) for i in range(steps)]
...
def _shot_locations_rink_figure(shots: pd.DataFrame | None = None) -> go.Figure:
    ...
    left_arc_x, left_arc_y = _arc_points(-corner_center_offset, corner_center_y, corner_radius, 180.0, 90.0)
    right_arc_x, right_arc_y = _arc_points(corner_center_offset, corner_center_y, corner_radius, 90.0, 0.0)
    ...
    crease_arc_x, crease_arc_y_points = _arc_points(0.0, goal_line_y, crease_arc_radius, crease_arc_start, crease_arc_end, steps=48)
    ...
    if not shot_df.empty:
        ...
        if not shot_df.empty:
            ...
            if xg_cap > 0:
                shot_df['xg_scaled'] = ((1.0 + 99.0 * capped_xg.div(xg_cap)).apply(math.log) / math.log(100.0))
            else:
            ...
            if not non_goal_df.empty:
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode='markers',
                        marker=dict(size=8, opacity=0.42, symbol='circle', color=non_goal_df['xg_scaled'], coloraxis='coloraxis', line=dict(width=0)),
                        customdata=non_goal_df[['outcome', 'xG', 'isRushLabel', 'isReboundLabel']].to_numpy(),
                        hovertemplate='Outcome: %{customdata[0]}<br>xG: %{customdata[1]:.3f}<br>Rush: %{customdata[2]}<br>Rebound: %{customdata[3]}<extra></extra>',
                        showlegend=False,
                    )
                )
            ...
            if not goal_df.empty:
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=goal_df['plot_y'],
                        mode='markers',
                        marker=dict(size=13, opacity=0.92, symbol='star', color=goal_df['xg_scaled'], coloraxis='coloraxis', line=dict(width=0)),
                        customdata=goal_df[['outcome', 'xG', 'isRushLabel', 'isReboundLabel']].to_numpy(),
                        hovertemplate='Outcome: %{customdata[0]}<br>xG: %{customdata[1]:.3f}<br>Rush: %{customdata[2]}<br>Rebound: %{customdata[3]}<extra></extra>',
                        showlegend=False,
                    )
                )
    else:
        xg_tick_vals = [0.0, 1.0 / 3.0, 2.0 / 3.0, 1.0]
        xg_tick_text = ['0.00', '0.03', '0.08', '0.20+']
        xg_colorscale = [
            [0.0, '#2166AC'],
            [0.25, '#67A9CF'],
            [0.5, '#D1E5F0'],
    ...
    _add_line(fig, -goal_line_half_width, goal_line_y, goal_line_half_width, goal_line_y, color=red_color, width=1.5, layer='above')
    ...
    _add_circle(fig, circle_x, circle_y, circle_r, line_color=red_color, width=1.4, layer='above')
    fig.add_trace(go.Scatter(x=net_x, y=net_y, mode='lines', line=dict(color='rgba(95,95,95,0.95)', width=1.1), fill='toself', fillcolor='rgba(95,95,95,0.70)', hoverinfo='skip', showlegend=False))
    fig.add_trace(go.Scatter(x=crease_x, y=crease_y, mode='lines', line=dict(color=red_color, width=1.2), fill='toself', fillcolor=crease_fill, hoverinfo='skip', showlegend=False))
    ...
    for sign in (-1, 1):
        _add_circle(fig, sign * 22.0, neutral_spot_y, faceoff_dot_radius, line_color=red_color, width=1.0, fill=red_color, layer='above')

    _add_line(fig, -11.0, goal_line_y, -14.0, y_max, color=red_color, width=1.2, layer='above')
    _add_line(fig, 11.0, goal_line_y, 14.0, y_max, color=red_color, width=1.2, layer='above')

    fig.update_layout(
        title=dict(text='Shot Locations Against', x=0.5, xanchor='center'),
        margin=dict(l=5, r=5, t=45, b=78),
        height=PLOT_H,
        uirevision='gsa-shot-locations',
        ...
        coloraxis=dict(
            ...
            colorbar=dict(
                ...
                outlinewidth=0,
                tickmode='array',
                tickvals=xg_tick_vals,
                ticktext=xg_tick_text,
    ...
    if shot_df.empty:
        fig.add_annotation(x=0.5, y=0.5, xref='paper', yref='paper', text='No shot attempts for this selection', showarrow=False, font=dict(size=14))
...
c_season, c_game, c_date, c_player, c_sit = st.columns(5, gap='small', vertical_alignment='top')

with c_season:
    season_label = st.selectbox('Season', SEASON_OPTIONS, index=DEFAULT_SEASON_INDEX, key='gsa_season_label')

season_id = SEASON_LABELS[season_label]
season_games, season_goalie_games, season_goalie_shots = _load_page_data(season_id)
...
prev_game_type = st.session_state.get('gsa_game_type_label')
...
with c_game:
    if st.session_state.get('gsa_game_type_label') not in available_game_types:
        st.session_state['gsa_game_type_label'] = available_game_types[game_type_index]
    game_type_label = st.selectbox('Game Type', available_game_types, index=game_type_index, key='gsa_game_type_label')
...
date_range_key = 'gsa_date_range_saved'
date_range_widget_key = 'gsa_date_range_widget'
date_context_key = 'gsa_date_range_context'
...
with c_sit:
    situation_label = st.selectbox('Situation', list(SITUATIONS.keys()), index=0, key='gsa_situation_label')
...
filtered_goalie_games = season_goalie_games.loc[season_goalie_games['gameId'].isin(selected_game_ids)].copy()
filtered_goalie_totals = _aggregate_goalies(filtered_goalie_games)
eligible_goalies = _eligible_goalies(filtered_goalie_totals, situation_code)

eligible_ids = eligible_goalies['playerId'].astype(int).tolist() if not eligible_goalies.empty else []
eligible_id_set = set(eligible_ids)
player_options = sorted(eligible_ids, key=lambda player_id: (_safe_name(name_by_player_id, player_id).lower(), player_id))

default_player_id = _default_goalie_id(eligible_goalies, situation_code)
...
with c_player:
    if player_options:
        ...
        st.selectbox('Player', options=['N/A'], index=0, disabled=True, key='gsa_player_id_empty')
        player_id = None

st.session_state['gsa_player_id_saved'] = player_id

player_row = None
if player_id is not None and not eligible_goalies.empty:
    match = eligible_goalies.loc[eligible_goalies['playerId'] == int(player_id)]
    if not match.empty:
        player_row = match.iloc[[0]].copy()

metric_series_map = {
    metric_name: _goalie_metric_series(eligible_goalies, metric_name, situation_code)
...
for metric_name, values in metric_series_map.items():
    if player_row is None:
        ...
        continue
    player_value = float(_goalie_metric_series(player_row, metric_name, situation_code).iloc[0])
    player_metric_values[metric_name] = player_value
    player_metric_deltas[metric_name] = _goalie_delta(values, player_value, metric_name)
...
with c_hs:
    with st.container(border=True):
        if player_id is not None:
            _img(f'assets/headshots/{int(player_id)}.png', fallback='assets/headshots/default.png')
        else:
            st.empty()

for column, metric_name in zip([c_m1, c_m2, c_m3, c_m4], DISPLAY_METRICS):
    with column:
        with st.container(border=True):
            st.metric(
                _metric_label(metric_name),
                value=_goalie_metric_value(metric_name, player_metric_values.get(metric_name, 0.0)),
...
with c1:
    with st.container(border=True):
        if player_row is None:
            st.info('Select a goalie with shots against in the selected date range.')
        else:
            corsi = float(_goalie_metric_series(player_row, 'CorsiA', situation_code).iloc[0])
            fenwick = float(_goalie_metric_series(player_row, 'FenwickA', situation_code).iloc[0])
            shot = float(_goalie_metric_series(player_row, 'ShotsA', situation_code).iloc[0])
            goal = float(_goalie_metric_series(player_row, 'GoalsA', situation_code).iloc[0])

            if corsi <= 0:
                st.info('No shot attempts against for this selection.')
            else:
                ...
                node_labels = [
                    ...
                    for name, value in zip(
                        ['CorsiA', 'Blocked', 'FenwickA', 'Missed', 'ShotsA', 'Saved', 'GoalsA'],
                ...
                fig.update_layout(title=dict(text='Volume & Outcome', x=0.5, xanchor='center'), margin=dict(l=10, r=10, t=50, b=10), height=PLOT_H)
                st.plotly_chart(fig, width='stretch', config={'displayModeBar': True})

with c2:
    with st.container(border=True):
        shot_scope = season_goalie_shots.loc[season_goalie_shots['gameId'].isin(selected_game_ids)].copy()
        shot_scope = shot_scope.loc[shot_scope['strengthState'].isin(_sbss_strength_values(situation_code))].copy()
        if player_id is not None:
            shot_scope = shot_scope.loc[shot_scope['goaliePlayerId'] == int(player_id)].copy()
        else:
            shot_scope = shot_scope.iloc[0:0].copy()

        st.plotly_chart(_shot_locations_rink_figure(shot_scope), width='stretch', config={'displayModeBar': True})

with c3:
    with st.container(border=True):
        if eligible_goalies.empty:
            st.info('No scatterplot data available for this selection.')
        else:
            available_metrics = SCATTER_METRICS.copy()
            prev_x_metric = st.session_state.get('gsa_scatter_x_metric')
            prev_y_metric = st.session_state.get('gsa_scatter_y_metric')

            if prev_x_metric in available_metrics:
                x_index = available_metrics.index(prev_x_metric)
            elif 'xGoalsA' in available_metrics:
                x_index = available_metrics.index('xGoalsA')
            else:
                x_index = 0

            if prev_y_metric in available_metrics:
                y_index = available_metrics.index(prev_y_metric)
            elif 'GoalsA' in available_metrics:
                y_index = available_metrics.index('GoalsA')
            ...
            with c_scx:
                x_metric = st.selectbox('X Axis', options=available_metrics, index=x_index, format_func=_metric_label, key='gsa_scatter_x_metric')
            with c_scy:
                y_metric = st.selectbox('Y Axis', options=available_metrics, index=y_index, format_func=_metric_label, key='gsa_scatter_y_metric')

            scatter_df = eligible_goalies[['playerId']].copy()
            scatter_df['x_val'] = _goalie_metric_series(eligible_goalies, x_metric, situation_code)
            scatter_df['y_val'] = _goalie_metric_series(eligible_goalies, y_metric, situation_code)
            scatter_df = scatter_df.dropna(subset=['playerId', 'x_val', 'y_val']).copy()

            if scatter_df.empty:
                st.info('No valid values for this selection.')
            else:
                scatter_df['name'] = scatter_df['playerId'].apply(lambda goalie_id: _safe_name(name_by_player_id, goalie_id))
                scatter_df['is_goalie'] = scatter_df['playerId'].eq(player_id) if player_id is not None else False

                others = scatter_df.loc[~scatter_df['is_goalie']].copy()
                mine = scatter_df.loc[scatter_df['is_goalie']].copy()
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=others['y_val'],
                        mode='markers',
                        marker=dict(size=7, opacity=0.55, color='rgba(160,160,160,0.65)', symbol='circle'),
                        customdata=others[['name']].to_numpy(),
                        hovertemplate='Player: %{customdata[0]}<br>' + f'{_metric_label(x_metric)}: %{{x:.1f}}<br>{_metric_label(y_metric)}: %{{y:.1f}}<extra></extra>',
                        showlegend=False,
                    )
                )
                ...
                if not mine.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            y=mine['y_val'],
                            mode='markers',
                            marker=dict(size=14, opacity=1.0, symbol='star', color='yellow'),
                ...
                fig.update_layout(
                    title=dict(text='Metrics vs. Goalies', x=0.5, xanchor='center'),
                    margin=dict(l=10, r=10, t=45, b=10),
                    xaxis=dict(title=_metric_label(x_metric)),
                    yaxis=dict(title=_metric_label(y_metric)),
