...
def _load_page_data(season_id: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    goalie_games = basic.merge(advanced, on=['playerId', 'gameId'], how='left', suffixes=('', '_adv'))
...
prev_game_type = st.session_state.get('geg_game_type_label')
...
with c_game:
    if st.session_state.get('geg_game_type_label') not in available_game_types:
        st.session_state['geg_game_type_label'] = available_game_types[game_type_index]
    game_type_label = st.selectbox('Game Type', available_game_types, index=game_type_index, key='geg_game_type_label')
...
date_range_key = 'geg_date_range_saved'
date_range_widget_key = 'geg_date_range_widget'
date_context_key = 'geg_date_range_context'
