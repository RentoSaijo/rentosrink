...
Historically, the expected-goals framework grew first in soccer, where analysts formalized shot-quality modeling from event data. Hockey adopted the same probabilistic logic and adapted it to a faster, more state-dependent game. As data quality improved, methods moved from simple interpretable baselines to more flexible nonlinear approaches, but the objective stayed the same: produce probabilities that are informative, stable, and well-calibrated. Calibration is critical because a model can rank chances well yet still be mis-scaled in absolute probability terms, making totals and comparisons less reliable. Good xG work is therefore not just model choice; it is disciplined data handling, explicit assumptions, and rigorous testing. This article follows that philosophy from foundations through implementation, evaluation, interpretation, and limitations so the model is understood as a full system, not a black-box number.
'''
)

st.markdown('## II. Architecture')
st.markdown(
    'This section is the build phase of the xG pipeline: define the training data, split the problem into the right game-state partitions, decide how feature information enters through versioning, and then choose the model class that can support repeated retraining without sacrificing predictive quality.'
)

st.markdown('### A. Data')
st.markdown('Training uses NHL play-by-play data pulled through `nhlscraper`, specifically `gc_pbps()`, with regular season and playoff shot events (`goal`, `shot-on-goal`, `missed-shot`) as the supervised sample. The model-training scripts fit on 2022-23 through 2024-25, while this article repeatedly compares three anchor seasons that serve different validation roles: 2021-22 is an unseen past check (backward out-of-sample), 2023-24 is a seen in-window reference, and 2025-26 is an unseen future check (forward out-of-sample). Keeping those roles fixed throughout the article makes each comparison interpretable as either in-era fit quality or out-of-era robustness.')
