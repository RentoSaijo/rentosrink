...
with c_season:
    season_label = st.selectbox('Season', season_options, index=0, key='seg_season_label')
...
prev_game_type = st.session_state.get('seg_game_type_label')
...
with c_game:
    if st.session_state.get('seg_game_type_label') not in available_game_types:
        st.session_state['seg_game_type_label'] = available_game_types[game_type_index]
    game_type_label = st.selectbox('Game Type', available_game_types, index=game_type_index, key='seg_game_type_label')
...
date_range_key = 'seg_date_range_saved'
date_range_widget_key = 'seg_date_range_widget'
date_context_key = 'seg_date_range_context'
...
with c_sit:
    situation_label = st.selectbox('Situation', list(SITUATIONS.keys()), index=0, key='seg_situation_label')
with c_cat:
    category_label = st.selectbox('Category', list(CATEGORIES.keys()), index=0, key='seg_category_label')
...
def _ranking_table(position_group: str) -> pd.DataFrame:
    source = player_totals.loc[player_totals['positionGroup'] == position_group].copy()
    source = _filter_bottom_minutes_quantile(source)
    table = source[display_cols].copy()
    if table.empty:
        return table
    ...
    table['oxG%'] = table['oxG%'].map(lambda value: round(float(value), 1))
    return table


c_forwards, c_defense = st.columns(2, gap='small', vertical_alignment='top')

with c_forwards:
    st.subheader('Forwards')
    forwards = _ranking_table('Forwards')
    if forwards.empty:
        st.info('No forwards available for this selection.')
    else:
        st.dataframe(forwards, width='stretch', hide_index=True, height=TABLE_HEIGHT)

with c_defense:
    st.subheader('Defensemen')
    defense = _ranking_table('Defensemen')
    if defense.empty:
        st.info('No defensemen available for this selection.')
    else:
        st.dataframe(defense, width='stretch', hide_index=True, height=TABLE_HEIGHT)
