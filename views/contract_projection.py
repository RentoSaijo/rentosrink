...
st.markdown('An NHL player contract is defined by two numbers that carry most of the strategic weight: term determines how long the commitment runs, and AAV determines how much cap space is consumed each season. Teams are not just pricing talent; they are buying a time path of risk and flexibility. A short term with high AAV protects optionality but pressures the near-term cap, while longer terms lower annual pressure in some cases but increase exposure to aging, role change, and performance variance.')
st.markdown('That tradeoff is amplified by a hard salary cap. Each contract decision competes directly with alternatives elsewhere on the roster, so errors are rarely isolated. If term is too long, the downside can compound over multiple seasons; if AAV is misaligned, the opportunity cost appears immediately in lineup depth and future maneuverability. Contract analysis therefore has to be framed as constrained resource allocation under uncertainty, not as a single fairness estimate.')
...
st.markdown(f'That leakage-safe window still preserves directional signal because we keep both level and trend features across offensive, defensive, usage, and player-contract context families. In the current snapshot, we train on {train_contracts} contracts from {train_skaters} skaters across {train_range} with {train_predictors} engineered predictors, hold out {validate_contracts} contracts from {validate_season} for model comparison, and score {test_rows} forward scenario rows for {test_skaters} skaters.')
...
st.markdown('Re-sign status changes the feasible contract space. In this pipeline, scenario generation enforces a different term ceiling by branch: re-sign cases evaluate terms 1 through 8, while non-re-sign cases evaluate terms 1 through 7. That is a hard structural guardrail, not a soft preference, and it prevents the model from proposing open-market structures that are not valid under the current rule set.')
